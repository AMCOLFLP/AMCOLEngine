...
function analyzeRole(unit){
  ...
  fb.textContent=`Technical keyword check: ${found}/${total}. Word count: ${words}. ${lengthOk?'Length is acceptable.':'Try to write at least 40 words or a multi-line exchange.'}`;
  updateProgress();
}
...
function checkDecision(unit){
  ...
  const fb=card.querySelector('.feedback');
  fb.className='feedback show '+(correct===total?'ok':'bad');
  fb.textContent=`Score: ${correct}/${total}. ${correct===total?'Strong operational decisions.':'Review the situations and choose the most precise term or procedure.'}`;
  updateProgress();
}
...
function speakEngineeringScript(id){
  const el=document.getElementById(id); if(!el) return;
  speakText(el.dataset.speech||el.textContent||'');
}
function speakText(text){
  if(!('speechSynthesis' in window)){alert('Speech playback is not supported in this browser.');return;}
  window.speechSynthesis.cancel();
  const utter=new SpeechSynthesisUtterance(text); utter.lang='en-US'; utter.rate=.86; utter.pitch=1;
  window.speechSynthesis.speak(utter);
}
function stopEngineeringSpeech(){if('speechSynthesis' in window) window.speechSynthesis.cancel();}
