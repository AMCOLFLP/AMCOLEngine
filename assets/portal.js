...
function renderUserTools(session){
  ...
  tools.innerHTML=`<button class="thai-toggle" id="thaiToggle" type="button" aria-pressed="false" title="Show or hide contextual Thai explanations"><span>TH</span><strong>Thai Assist</strong></button><div class="portal-user-chip" role="status" aria-label="Signed in as ${session.displayName}, ${roleLabel(session.role)}" title="Signed in as ${session.displayName} (${roleLabel(session.role)})"><span class="portal-avatar" aria-hidden="true">${session.role==='admin'?'A':session.role==='teacher'?'T':'S'}</span><div><span class="portal-signed-in">Signed in</span><strong>${session.displayName}</strong><small>${roleLabel(session.role)}</small></div></div><button class="portal-logout" id="portalLogoutBtn" type="button" aria-label="Log out of the AMCOL Maritime English for Engineers portal" title="Log out"><span class="portal-logout-icon" aria-hidden="true">↪</span><span class="portal-logout-label">Log out</span></button>`;
